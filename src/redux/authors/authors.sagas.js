...
export function* fetchAuthorAsync({ payload }) {
  try {
    const authorResponse = yield fetch(`https://openlibrary.org/authors/${payload}.json`);
    const author = yield authorResponse.json();
    const authorName = author.name.toLowerCase().replace(/-_ /g, '+');
    const worksResponse = yield fetch(`https://openlibrary.org/search.json?author=${authorName}`);
